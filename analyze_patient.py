...
def visualize_sensor_data(patient_dataframes, save_dir, patient_id="1023"):
    ...
    if 'activity' in patient_dataframes and not patient_dataframes['activity'].empty:
        ...
        if 'start_time' in activity_df.columns and 'end_time' in activity_df.columns:
            ...
            for activity_type in unique_activities:
                ...
                sizes = activity_subset['duration'] * 2 # make bigger so you can see 
            ...
            plt.xlabel('Time')
            plt.yticks(range(len(unique_activities) + 1), list(unique_activities) + ['Available'])
            plt.xticks(rotation=45)
            plt.grid(True, alpha=0.3)
            ...
            plt.text(0.5, 0.5, 'No Activity data structure', ha='center', va='center', transform=ax.transAxes, fontsize=14, color='red')
    else:
        plt.text(0.5, 0.5, 'No Activity data', ha='center', va='center',  transform=ax.transAxes, fontsize=14, color='red')
    ...
    if 'sleep' in patient_dataframes and not patient_dataframes['sleep'].empty:
        ...
        plt.text(0.5, 0.5, 'No Sleep data', ha='center', va='center',  transform=ax.transAxes, fontsize=14, color='red')
    plt.tight_layout()
...
def create_data_availability_timeline(patient_dataframes, patient_id, save_dir):
    ...
    if sensor_info:
        ...
        for i, (sensor, color) in enumerate(zip(sensor_info, colors)):
            ...
            plt.text(mid_time, i, f"{sensor['count']}", ha='center', va='center', fontweight='bold', fontsize=10)
        if overlap_start <= overlap_end:
