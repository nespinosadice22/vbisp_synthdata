    ...
    try:
        ...
        for sensor_key, time_col in sensor_configs:
            if sensor_key in patient_data and not patient_data[sensor_key].empty:
                stats = calculate_sensor_intervals(patient_data[sensor_key], time_col, sensor_key, patient_id)
                if stats:
...
def create_population_interval_plots(all_results, save_dir):
    ...
    # mean interval distributions by sensor
    plt.subplot(3, 3, 1)
    ...
    plt.grid(True, alpha=0.3)
    # number of measurements per patient by sensor
    plt.subplot(3, 3, 3)
...
def create_individual_sensor_histograms(df_results, save_dir):
    ...
    if n_rows == 1:
        axes = axes.reshape(1, -1) #bug fix - ensure 2D array 
    for i, sensor in enumerate(sensors):
        row = i // n_cols
        col = i % n_cols
        ax = axes[row, col]
        # all intervals from all patients for this sensor
        all_intervals = []
        sensor_data = df_results[df_results['sensor'] == sensor]
...
def main():
    ...
    # fix: multiprocessing for faster analysis
    ...
    # check blood glucose missing data 
    ...
    #missing blood glucose patients analysis
    ...
    # save missig files to direcetory 
